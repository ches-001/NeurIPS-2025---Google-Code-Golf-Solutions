def p(a):
 n=[];f=[divmod(sum(a,[]).index(1),len(a[0]))]
 for y,x in f:
  if 0<=x<len(a[0]) and 0<=y<len(a) and (y,x) not in n and a[y][x]:
   n+=(y,x),
   for i in-1,0,1:
    for j in-1,0,1:f+=(y+i,x+j),
 Y,X=zip(*n);o=[];H=[r[min(X):max(X)+1]for r in a[min(Y):max(Y)+1]];H=[*zip(*H)];o+=H,;H=H[::-1];o+=H,;H=[*zip(*H)];o+=H,;H=H[::-1];o+=H,;H=[*zip(*H)];o+=H,;H=H[::-1];o+=H,;H=[*zip(*H)];o+=H,;H=H[::-1];o+=H,
